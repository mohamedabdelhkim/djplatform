...
export const onRequestPost: PagesFunction<Env> = async (context) => {
  try {
    ...
    // PHASE 1 - the widget is on the page but its result is not enforced yet.
    // Enforcing before confirming the widget renders is exactly what took the
    // booking form down for forty minutes on 6 Sep: no widget means no token,
    // and a fail-closed check then rejects every genuine submission. The
    // verifyTurnstile() helper below stays in place and is switched on in a
    // follow-up commit, once the widget is confirmed rendering in a browser.
