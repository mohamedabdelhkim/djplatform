...
**Currently staged, not enforced.** The widget renders and sends a token, but
`verifyTurnstile()` is not called yet — enforcing before the widget was confirmed
rendering is what took the booking form down for forty minutes on 6 September.
Enforcement is switched on in a follow-up commit once a browser confirms the
challenge resolves.

The site key is public and lives in `BookingSection.tsx`; the secret is a Pages
secret. Once enforced, verification **fails closed**: a missing secret, a missing token or a
